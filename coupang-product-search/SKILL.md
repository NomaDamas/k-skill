...
2. **Credentialless hosted fallback path** — 위 두 키 중 하나라도 없는 경우(또는 `OPENCLAW_SHOPPING_FORCE_HOSTED=1`). upstream이 자동으로 Retention Corp의 hosted 백엔드(`https://a.retn.kr/v1/public/assist`)로 떨어진다. 이 경로는 `X-OpenClaw-Client-Id` allowlist로 게이트되어 있으며, 기본값은 `openclaw-skill`, k-skill 전용 allowlist 값은 `coupang-mcp-fallback`이다.
...
| `OPENCLAW_SHOPPING_CLIENT_ID` | hosted fallback이 보낼 `X-OpenClaw-Client-Id`. k-skill 권장값 `coupang-mcp-fallback`. | `openclaw-skill` |
...
### 4. (optional) hosted fallback 강제 또는 allowlist 지정

hosted fallback을 강제하거나 k-skill 전용 allowlist 값을 쓸 때 환경변수를 export한다.

```bash
export OPENCLAW_SHOPPING_FORCE_HOSTED=1
export OPENCLAW_SHOPPING_CLIENT_ID=coupang-mcp-fallback
