...
- 미국 재무부 일별 par yield curve CSV: https://home.treasury.gov/resource-center/data-chart-center/interest-rates/TextView?type=daily_treasury_yield_curve — `morning-market-briefing`의 `yields` helper가 같은 host의 `/daily-treasury-rates.csv/<YYYY>/all?type=daily_treasury_yield_curve&field_tdr_date_value=<YYYY>&page&_format=csv`를 직접 호출한다. 헤더는 `Date,"1 Mo",... ,"10 Yr","20 Yr","30 Yr"`, 값은 퍼센트, 날짜는 `MM/DD/YYYY`, 행은 최신순. 무인증 공개 read-only이므로 `k-skill-proxy`를 경유하지 않는다. 2026-09-16 실측에서 응답이 19초대까지 걸렸으므로 helper 기본 타임아웃은 30초다.
- FRED 공개 CSV: https://fred.stlouisfed.org/graph/fredgraph.csv?id=<ID1>,<ID2> — `morning-market-briefing`의 `series` helper가 지수·VIX·환율·원자재 시계열을 직접 호출한다. 2026-09-16 실측에서 (1) `cosd`/`coed` 기간 파라미터를 무시하고 전체 이력을 반환했고(11,852행), (2) 일부 요청은 `Content-Type: application/zip`으로 오며 ZIP 내부에 시리즈별 CSV가 나뉘어 들어 있었다. helper는 ZIP 멤버를 전부 날짜 기준으로 병합하고, 요청한 시리즈가 응답에 없으면 `warnings`에 명시한다. 시리즈별 발표 지연이 달라(같은 날 기준 지수 1일, VIX 2일, 원/KRW 5일, WTI 7일) 관측일 대비 `stale`·`lag_days`를 함께 보고한다.
- JS 챌린지로 보호되는 시장 데이터 집계 사이트는 `morning-market-briefing`의 조회 경로에서 제외하고 공식 정적 endpoint를 우선한다.
