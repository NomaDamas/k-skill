...
웹사이트를 조회하거나 크롤링하는 스킬은 처음부터 특정 화면 구조나 임시 우회법을 고정하지 않는다. 먼저 `insane-search`식 접근처럼 **사이트에 상관없이 반복 가능한 탐색 절차**를 적용해 공개적으로 사용할 수 있는 안정적인 접근 경로를 찾고, 그 결과만 해당 스킬에 site-dependent 지식으로 패키징한다.
...
5. **발견한 site-dependent 지식만 스킬에 고정하기**: 탐색 과정에서 확인한 검색 URL, 필수 파라미터, 결과 해석 규칙, fallback 순서를 `SKILL.md`와 패키지 코드에 좁게 기록한다.
