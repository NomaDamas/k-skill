...
- DART API 요청 한도: 공식 가이드 기준 "일반적으로 20,000건 이상의 요청"에 대해 `020` (요청 제한 초과)이 발생함. 분당 throttle 등 세부 제약은 공개 가이드에 명시되지 않음. 본인 키의 정확한 사용 현황은 로그인 후 OpenDART 사이트의 [오픈API 이용현황](https://opendart.fss.or.kr/mng/apiUsageStatusView.do) 페이지에서 확인 가능.
- 상장폐지·오래된 비상장 법인은 데이터가 없을 수 있음
- `list.json`의 `corp_name` 파라미터는 검색 필터로 동작하지 않음. 회사명을 기준으로 좁히려면 `corpCode.xml` ZIP을 받아 `corp_code`(8자리 고유번호)를 먼저 확보한 뒤 `corp_code` 로 호출. `corp_code` 자체는 선택사항이지만, 미지정 시 검색 기간이 3개월 이내로 제한됨.
