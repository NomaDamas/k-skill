...
검색 결과에는 매입글/광고/악세서리 글이 섞일 수 있으므로, 가격·모델·지역·설명 충실도를 기준으로 상위 후보만 추린다.
...
상세에서는 `price`, `description`, `location`, `category`, `status`, `sellerName`, `sellerReviewCount`, `favoriteCount`, `transportUsed` 를 우선 본다.
