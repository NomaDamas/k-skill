...
- **한글 이용약관 템플릿 0바이트 (#660)**: 현재 pin 시점 기준 업스트림의 `jurisdictions/kr-pipa/terms-of-service.ko.mdx.tmpl` 과 `templates/terms-of-service.mdx.tmpl` 이 0바이트라 한국어 경로에서 이용약관 페이지가 생성되지 않는다. 개인정보처리방침(ko/en)과 영문 약관 페이지는 정상 생성된다. 업스트림에 템플릿이 채워지면 pin bump 로 해소하며, `scripts/install.sh` 가 설치 시 0바이트 템플릿을 경고로 표면화한다.
