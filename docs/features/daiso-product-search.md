...
- 특정 매장의 **매장 픽업 재고** 확인
- 매장 픽업 재고가 `Unauthorized` 로 차단되면 차단 상태를 명확히 표시하고, 필요하면 온라인 재고 참고값 함께 확인
...
- 매장 픽업 재고가 `Unauthorized` 로 차단된 경우에는 `다이소몰이 현재 매장 픽업 재고 API를 차단해 정확한 매장 재고 수량은 확인할 수 없다`고 답하고, 온라인 재고는 참고값으로만 구분합니다.
