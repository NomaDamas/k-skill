...
브라우저 세션이 필요한 스킬(`hipass-receipt`, `court-auction-notice-search`, `court-payment-order-assistant`)은 `k-skill-browser-runtime`을 기본 런타임으로 쓴다. 런타임은 CDP-only로 사용자가 직접 띄운 BrowserOS GUI 세션에 붙으며, BrowserOS를 launch하거나 headless로 띄우지 않는다. 자세한 작성 가이드는 [브라우저 런타임 문서](browser-runtime.md)와 [새 스킬 추가 가이드](adding-a-skill.md)를 참고.
...
| 하이패스 영수증 발급 | 사용자 시크릿 불필요 (기본 `auto`: BrowserOS CDP `http://127.0.0.1:9100` 우선 → Chrome CDP `http://127.0.0.1:9222` fallback 세션에서 수동 로그인, [브라우저 런타임](browser-runtime.md) 참고) |
