...
> ## ⚠️ 근처 블루리본 맛집 스킬 — 지원 중단 (우회 방법 발견)
>
...
>
> ### 🔓 우회 방법 발견 (`rebrowser-playwright`)
>
> [`playwright-bot-bypass`](https://github.com/greekr4/playwright-bot-bypass) 스킬을 사용하면 `bluer.co.kr`에 **정상 접근(HTTP 200)** 이 가능합니다. 일반 Playwright가 403으로 차단되는 반면, `rebrowser-playwright` + 8개 stealth 패치 조합으로 잠실 지역 맛집 225개 목록까지 정상 로딩됨을 확인했습니다.
>
> | 항목 | rebrowser-playwright | 일반 Playwright |
> |------|---------------------|----------------|
> | HTTP | **200 OK** | **403 Forbidden** |
> | `navigator.webdriver` | `undefined` | `true` |
> | WebGL | Apple M2 (실제 GPU) | SwiftShader |
>
> 설치: `npx skills add greekr4/playwright-bot-bypass`
>
> 자세한 내용은 [#92](https://github.com/NomaDamas/k-skill/issues/92)를 참고하세요.
