...
| 문서 → Markdown 변환 | PDF·Word·Excel·PowerPoint·HTML 등 다양한 파일을 Markdown으로 변환 (Microsoft MarkItDown) | 불필요 | [문서 변환 가이드](docs/features/markitdown.md) |
| 프로젝트 헬스 체크 | 기존 프로젝트 Infrastructure·Security·Quality·Harness 4개 축 점검 및 갭 리포트 | 불필요 | [project-check](project-check/SKILL.md) |
| 프로젝트 스캐폴딩 | 인터뷰 기반 CLAUDE.md·ROADMAP·.gitignore·.env.example 생성 | 불필요 | [project-init](project-init/SKILL.md) |
| Claude Code 인프라 설정 | ai-constitution·agents.md·hooks·memory 등 에이전트 오케스트레이션 레이어 구성 | 불필요 | [harness-init](harness-init/SKILL.md) |
| 에이전트팀 구성 | orchestrator·reviewer·implementer 등 Claude Code 에이전트팀 영구 설치 | 불필요 | [team-init](team-init/SKILL.md) |
| pre-push 보안 게이트 | push 전 시크릿 스캔·빌드·테스트·코드 리뷰 자동 실행 | 불필요 | [pre-push](pre-push/SKILL.md) |
