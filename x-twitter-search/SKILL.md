...
description: Xquik REST API로 X(Twitter) 공개 게시물 검색, 게시물 조회, 공개 프로필 확인을 수행하는 읽기 전용 스킬.
license: MIT
metadata:
  category: research
  locale: ko-KR
...
```bash
npx -y @nomadamas/k-skill@0 instruct x-twitter-search
```

The CLI detects the current runtime (Dolshoi vault/CloakBrowser vs generic) and prints only the applicable instructions, always up to date. Helper files bundled with the CLI are listed by:

```bash
npx -y @nomadamas/k-skill@0 files x-twitter-search
```

If `npx` is unavailable, install Node.js 18+ or follow https://github.com/NomaDamas/k-skill#readme, or read the source instructions at https://github.com/NomaDamas/k-skill/blob/main/x-twitter-search/instruction.md.
...
```bash
npx -y @nomadamas/k-skill@0 read x-twitter-search references/DISCLAIMER.md
```
