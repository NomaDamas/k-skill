...
const { normalizeCustomsQuery, fetchCustomsCargo } = require("./customs-cargo-clearance");
...
function buildServer({ env = process.env, provider = null, now = () => new Date() } = {}) {
  ...
  app.get("/v1/customs/cargo-clearance", async (request, reply) => {
    let normalized;
    try {
      normalized = normalizeCustomsQuery(request.query || {});
    } catch (error) {
      reply.code(400);
      return { error: "bad_request", message: error.message };
    }
    const cacheKey = makeCacheKey({ route: "customs-cargo-clearance", ...normalized });
    const cached = cache.get(cacheKey);
    ...
    const result = await fetchCustomsCargo({ ...normalized, serviceKey: config.molitApiKey });
    if (result.error) {
