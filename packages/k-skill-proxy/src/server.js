...
const { normalizeCareernetQuery, fetchCareernet } = require("./careernet");
...
function buildConfig(env = process.env) {
  return {
    ...
    careernetApiKey: trimOrNull(env.CAREERNET_API_KEY ?? env.KSKILL_CAREERNET_API_KEY),
...
function buildServer({ env = process.env, provider = null, now = () => new Date() } = {}) {
  ...
  app.get("/v1/careernet/career/:operation", async (request, reply) => {
    let normalized;
    try { normalized = normalizeCareernetQuery(request.params.operation, request.query || {}); }
    catch (error) { reply.code(400); return { error: "bad_request", message: error.message }; }
    const cacheKey = makeCacheKey({ route: `careernet-${request.params.operation}`, ...normalized });
    const cached = cache.get(cacheKey);
    if (cached) return { ...cached, proxy: { ...cached.proxy, cache: { hit: true, ttl_ms: config.cacheTtlMs } } };
    if (!config.careernetApiKey) {
      reply.code(503);
      return { error: "upstream_not_configured", message: "CAREERNET_API_KEY is not configured on the proxy server.", proxy: { name: config.proxyName, cache: { hit: false, ttl_ms: config.cacheTtlMs } } };
    }
    const result = await fetchCareernet({ ...normalized, apiKey: config.careernetApiKey });
