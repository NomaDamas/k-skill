...
test("disconnectBrowser disconnects and refuses unsafe close fallback", async () => {
  ...
  const closeOnly = { async close() { throw new Error("must not close") } }
  await assert.rejects(
    () => runtime.disconnectBrowser(closeOnly),
    /refusing to close a potentially user-owned browser/
  )
})
